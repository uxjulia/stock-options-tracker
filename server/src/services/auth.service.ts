...
import { JWT_ACCESS_EXPIRY, JWT_REFRESH_EXPIRY, BCRYPT_ROUNDS } from '../config/constants';
...
export function listUsers(): PublicUser[] {
  const rows = db.prepare('SELECT id, username, created_at FROM users ORDER BY created_at ASC').all();
...
export function deleteUser(targetId: number, requesterId: number): void {
  if (targetId === requesterId) throw new SelfDeleteError('Cannot delete your own account');
...
export async function adminResetPassword(targetId: number, newPassword: string): Promise<void> {
  const existing = db.prepare('SELECT id FROM users WHERE id = ?').get(targetId);
  if (!existing) throw new UserNotFoundError('User not found');
