...
function AddUserTab() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(status === 409 ? 'Username already exists.' : (err?.response?.data?.error ?? 'Failed to create user.'));
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <div className="flex flex-col items-center gap-4 py-4">
        ...
        <p className="text-slate-300 text-center">
          User <span className="font-semibold text-slate-100">{successName}</span> created!
        </p>
        <Button onClick={reset} className="w-full">Add Another</Button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <Input label="Username" type="text" value={username}
        onChange={(e) => setUsername(e.target.value)} placeholder="Enter username" required autoComplete="off" />
  ...
  if (fetchError) {
    return (
      <div className="text-center py-8">
        ...
        <Button onClick={fetchUsers} variant="ghost" className="text-sm">Retry</Button>
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3">
              {/* Avatar */}
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isSelf ? 'bg-accent/20' : 'bg-slate-700'}`}>
                <span className={`text-xs font-semibold ${isSelf ? 'text-accent' : 'text-slate-300'}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {isSelf && (
                    <span className="text-[10px] font-semibold bg-accent/15 text-accent px-1.5 py-0.5 rounded-full">you</span>
                  )}
                </div>
              ...
              {!isSelf && (
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <button
                    onClick={() => {
                      if (isReseting) { setResetingId(null); setResetPassword(''); setResetError(''); }
                      else { setResetingId(u.id); setResetPassword(''); setResetError(''); }
                    }}
            ...
            {isReseting && (
              ...
              >
                <p className="text-xs text-slate-400">Set new password for <span className="font-medium text-slate-200">{u.username}</span></p>
                <Input
                  label=""
                  type="password"
                  value={resetPassword}
                ...
                <div className="flex gap-2">
                  <Button type="button" variant="ghost" className="flex-1 text-sm"
                    onClick={() => { setResetingId(null); setResetPassword(''); setResetError(''); }}>
                    Cancel
                  </Button>
                  <Button type="submit" className="flex-1 text-sm" disabled={resetLoading}>
...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Settings" size="sm">
      ...
      {activeTab === 'manage' && user && <ManageUsersTab currentUserId={user.id} />}
    </Modal>
  );
}
