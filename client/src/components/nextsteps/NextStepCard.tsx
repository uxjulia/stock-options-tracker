...
export function NextStepCard({ recommendation, onAddOption }: NextStepCardProps) {
  ...
  return (
    <div className="bg-bg-surface border border-slate-700/50 rounded-xl p-4 sm:p-5">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-lg font-bold text-slate-100">{recommendation.ticker}</span>
        ...
        {recommendation.recommendation !== 'neutral' && (
          <Badge
            variant={
              recommendation.recommendation === 'sell_covered_call' ? 'warning' : 'info'
            }
          >
      ...
      {!recommendation.is_ignored && recommendation.recommendation !== 'neutral' && (
        <div
      ...
      {recommendation.open_contracts_count > 0 && (
        <p className="text-xs text-slate-500 mb-3">
          ...
          {recommendation.open_contracts_count > 1 ? 's' : ''} currently active on this ticker.
        </p>
      )}

      <div className="flex gap-2">
        {!recommendation.is_ignored && recommendation.recommendation !== 'neutral' && onAddOption && (
          <Button
            size="sm"
            variant="primary"
