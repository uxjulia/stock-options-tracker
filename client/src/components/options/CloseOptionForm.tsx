...
export function CloseOptionForm({ isOpen, onClose, option }: CloseOptionFormProps) {
  ...
  const onSubmit = async (data: FormData) => {
    await closeOption.mutateAsync({
      ...
      data: {
        ...
        cost_to_close: data.close_reason === 'closed_early' ? data.cost_to_close : undefined,
      },
    });
    onClose();
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {closeReason && (
          <p className="text-xs text-slate-500">
            {CLOSE_REASON_DESCRIPTIONS[closeReason as keyof typeof CLOSE_REASON_DESCRIPTIONS]}
          </p>
        )}
        ...
        {closeReason === 'closed_early' && (
          <Input
            ...
            min="0.01"
            placeholder="0.45"
            hint={option.direction === 'sold' ? 'Amount paid to buy back' : 'Amount received from sale'}
        ...
        <div className="bg-bg-elevated border border-slate-700 rounded-lg p-3 space-y-1">
          <p className="text-xs text-slate-500 uppercase tracking-wide mb-2">Net Proceeds</p>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Premium {option.direction === 'sold' ? 'received' : 'paid'}</span>
            <span className="font-mono text-slate-300">
              {formatCurrency(option.premium * 100 * option.quantity)}
            </span>
          </div>
          {closeReason === 'closed_early' && costToClose && Number(costToClose) > 0 && (
            <div className="flex justify-between text-sm">
          ...
          <div className="flex justify-between text-sm border-t border-slate-700 pt-1 mt-1">
            ...
            <span className={`font-mono font-bold ${netProceeds >= 0 ? 'text-profit' : 'text-loss'}`}>
          ...
          <div className="bg-warning/10 border border-warning/30 rounded-lg p-3">
            <p className="text-xs text-warning">
              ...
              {option.direction === 'sold' && option.option_type === 'put'
                ...
                : option.direction === 'bought' && option.option_type === 'call'
        ...
        <div className="flex gap-3 pt-2">
          ...
          <Button type="submit" loading={closeOption.isPending} className="flex-1">
            Close Option
          </Button>
        </div>
      </form>
