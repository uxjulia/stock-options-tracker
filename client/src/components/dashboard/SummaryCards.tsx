...
export function SummaryCards() {
  const { data: openOptions, isLoading: loadingOptions } = useOptions({ status: 'open', limit: 200 });
  ...
  if (loadingOptions || loadingPnl) {
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-bg-surface border border-slate-700/50 rounded-xl p-4 sm:p-6 flex items-center justify-center h-24">
            <Spinner size="sm" />
          </div>
        ))}
      </div>
  ...
  const expiringThisWeek = (openOptions?.data ?? []).filter(
    (o) => calcDaysUntilExpiry(o.expiration_date) <= 7 && calcDaysUntilExpiry(o.expiration_date) >= 0
  ).length;
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <StatCard
        ...
        valueClass={winRate >= 70 ? 'text-profit' : winRate >= 50 ? 'text-warning' : 'text-loss'}
      />
      <StatCard
