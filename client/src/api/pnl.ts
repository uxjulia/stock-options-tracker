...
export async function getPnLByAccount(year?: number): Promise<PnLByAccount[]> {
  const res = await apiClient.get('/pnl/by-account', { params: year ? { year } : {} });
  return res.data;
}

export async function getPnLByTicker(params?: { account_id?: number; year?: number }): Promise<PnLByTicker[]> {
